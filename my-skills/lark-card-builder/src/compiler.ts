...
interface ValidationState {
  ...
  customTextSizes: Set<string>;
  sawAddressableElement: boolean;
  sawStreamTarget: boolean;
}
...
function pathFor(parts: readonly PropertyKey[]): string {
  ...
  switch (component.kind) {
    ...
    case "div":
      ...
      component.fields?.forEach((field, index) =>
        validateText(state, field.text, `${path}.fields[${index}].text`),
      );
      validateIcon(state, component.icon, `${path}.icon`);
      return;
    ...
    case "table": {
      ...
      validateColor(
        ...
        `${path}.headerStyle.textColor`,
        ["default"],
      );
      validateTextSize(
        state,
        component.headerStyle?.textSize,
        `${path}.headerStyle.textSize`,
      );
      return;
    }
...
function validateCardSpec(spec: CardSpec): ValidationState {
...
function put(target: JsonObject, key: string, value: JsonValue | undefined): void {
  if (value !== undefined) target[key] = value;
}

function compileText(text: TextSpec): JsonObject {
...
function compileIcon(icon: IconSpec): JsonObject {
  if (icon.kind === "customIcon") {
    return { tag: "custom_icon", img_key: icon.imageKey };
  }
  const output: JsonObject = { tag: "standard_icon", token: icon.token };
  put(output, "color", icon.color);
  put(output, "size", icon.size);
  return output;
}

function compileBehavior(behavior: BehaviorSpec): JsonObject {
...
function compileConfirm(confirm: { title: string; text?: string }): JsonObject {
  const output: JsonObject = {
...
function stableElementId(cardKey: string, key: string): string {
...
function applyAddressability(
  ...
  if (component.kind === "div" && component.streamTarget) {
    ...
    if (typeof text === "object" && text !== null && !Array.isArray(text)) {
      (text as JsonObject).element_id = elementId;
    }
  } else {
...
function compileMarkdownContent(
  component: Extract<ComponentSpec, { kind: "markdown" }>,
): string {
  if (component.content !== undefined) return component.content;
...
function compileComponent(
  ...
  switch (component.kind) {
    case "markdown": {
      ...
      if (component.icon) output.icon = compileIcon(component.icon);
      break;
    }

    case "div":
      output = { tag: "div" };
      if (component.text) output.text = compileText(component.text);
      if (component.fields) {
    ...
    case "image":
      ...
      put(
        output,
        "scale_type",
        component.scaleType === "cropCenter"
          ? "crop_center"
    ...
    case "button": {
      const appearanceMap: Record<NonNullable<typeof component.appearance>, string> = {
        default: "default",
        primary: "primary",
        danger: "danger",
        text: "text",
      ...
      if (component.confirm) output.confirm = compileConfirm(component.confirm);
    ...
    case "input":
      ...
      put(
        output,
        "input_type",
        component.inputType === "multilineText"
          ? "multiline_text"
          : component.inputType,
      );
      ...
      if (component.behaviors) output.behaviors = component.behaviors.map(compileBehavior);
      if (component.confirm) output.confirm = compileConfirm(component.confirm);
      break;
    ...
    case "collapsiblePanel": {
      ...
      put(
        header,
        "width",
        component.header.width === "autoWhenFold"
          ? "auto_when_fold"
      ...
      put(
        header,
        "icon_position",
        component.header.iconPosition === "followText"
          ? "follow_text"
...
export function countTaggedNodes(value: unknown): number {
  ...
  for (const child of Object.values(record)) count += countTaggedNodes(child);
  return count;
}

function compileHeader(header: HeaderSpec, suffix?: string): JsonObject {
  const title = compileText(header.title);
  if (suffix) title.content = `${header.title.content}${suffix}`;
  const output: JsonObject = { title };
  if (header.subtitle) output.subtitle = compileText(header.subtitle);
  ...
  if (header.icon) output.icon = compileIcon(header.icon);
  put(output, "padding", header.padding);
  return output;
}

function compileCardLink(link: CardLinkSpec): JsonObject {
  const output: JsonObject = { url: link.url };
...
function compileConfig(spec: CardSpec, forceStreaming: boolean): JsonObject {
  const config: JsonObject = { update_multi: true };
  ...
  if (forceStreaming || spec.config?.streamingMode) config.streaming_mode = true;
  if (spec.config?.streamingConfig) {
    const streaming: JsonObject = {};
  ...
  if (spec.config?.textStyles) {
    const textSize: JsonObject = {};
    for (const [name, token] of Object.entries(spec.config.textStyles)) {
      const compiled: JsonObject = { default: token.default };
...
function compileBodyLayout(spec: CardSpec, elements: JsonObject[]): JsonObject {
  const output: JsonObject = { elements };
...
function baseCount(spec: CardSpec, forceStreaming: boolean, syntheticHeader: boolean): number {
...
function packBlocks(
  ...
  const parts: CompiledBlock[][] = [];
  let current: CompiledBlock[] = [];
...
function compileBlocks(spec: CardSpec): CompiledBlock[] {
  return spec.blocks.map((block: BlockSpec, sourceIndex) => {
    const refs: CompiledElementRef[] = [];
...
function validateCallbackBytes(
  value: unknown,
  path: string,
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) =>
      validateCallbackBytes(entry, `${path}[${index}]`, maxCallbackBytes, issues),
    );
    return;
  }
...
export function compileCardSpec(
  input: unknown,
  ...
  if (!parsed.success) {
    return {
      ...
      issues: parsed.error.issues.map((zodIssue) => ({
        code:
          zodIssue.code === "too_big" && zodIssue.origin === "string"
            ? "field_too_long"
  ...
  if (maxCallbackBytes !== undefined) {
    const callbackIssues: CardIssue[] = [];
  ...
  const elementIndex: Record<string, {
    part: number;
    elementId: string;
    kind: ComponentSpec["kind"];
    streamable: boolean;
...
function countTag(value: unknown, tag: string): number {
  if (Array.isArray(value)) {
  ...
  for (const child of Object.values(record)) count += countTag(child, tag);
  return count;
}

export function buildOrThrowCardSpec(
  input: unknown,
  options: CompileOptions = {},
): Extract<CompiledCards, { ok: true }> {
  const result = compileCardSpec(input, options);
  if (!result.ok) throw new CardBuildError(result.issues);
  return result;
}
