...
type SupportedCatalogEntry = Extract<
  (typeof OFFICIAL_COMPONENT_CATALOG)[number],
  { status: "supported" }
>;

export type SupportedSpecKind = SupportedCatalogEntry["specKind"];
export type SupportedOfficialTag = SupportedCatalogEntry["tag"];
export type OfficialComponentTag =
  (typeof OFFICIAL_COMPONENT_CATALOG)[number]["tag"];

const supportedEntries = OFFICIAL_COMPONENT_CATALOG.filter(
  (entry): entry is SupportedCatalogEntry => entry.status === "supported",
);

export const SUPPORTED_COMPONENT_KINDS = supportedEntries.map(
  (entry) => entry.specKind,
) as SupportedSpecKind[];
export const SUPPORTED_SPEC_KINDS = SUPPORTED_COMPONENT_KINDS;
export const SUPPORTED_OFFICIAL_TAGS = supportedEntries.map(
  (entry) => entry.tag,
) as SupportedOfficialTag[];
export const UNSUPPORTED_OFFICIAL_TAGS = OFFICIAL_COMPONENT_CATALOG
  .filter((entry) => entry.status === "unsupported")
  .map((entry) => entry.tag);
export const OFFICIAL_COMPONENT_TAGS = OFFICIAL_COMPONENT_CATALOG.map(
  (entry) => entry.tag,
);
export const SPEC_KIND_TO_OFFICIAL_TAG = Object.fromEntries(
  supportedEntries.map((entry) => [entry.specKind, entry.tag]),
) as Record<SupportedSpecKind, SupportedOfficialTag>;
...
export const BUILDER_CAPABILITIES = {
  ...
  ) as { [K in keyof typeof OFFICIAL_LIMITS]: (typeof OFFICIAL_LIMITS)[K]["value"] },
} as const;
