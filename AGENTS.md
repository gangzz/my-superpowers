...
7. 更新 `.source`，其中 `targets` 必须记录新机器首次安装时需要保留的安装平台；再运行适用的结构检查和行为验证。
8. 展示变更和未解决问题，等待用户明确确认是否安装。安装命令会同时生成或刷新本机发布快照。

决策记录写入后不得自动执行 Git commit。

## 安装与发布快照规则

“开发完成”和“确认安装”是两个不同状态。发布快照保留为开发版与运行版之间的稳定边界，但不再提供独立的发布动作。

安装是显式的本机晋级动作，不是上传动作。Git 只同步 `my-skills/`、决策、测试、依赖锁和安装工具；另一台电脑拉取源码后，在当地重新执行安装，由安装命令重建发布快照。

只有用户明确要求安装时，才可以更新 `published-skills/<skill-name>/` 和平台安装入口。安装时：
...
- 验证发布目录，而不只验证开发目录。
- 不自动提交 Git、创建标签或推送；这些动作分别需要用户明确授权。发布目录本身即使已生成，也不得加入 Git。
