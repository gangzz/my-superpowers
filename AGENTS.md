...
- `my-skills/<skill-name>/`：个人 Skill 的开发和调试版本，也是 Git 中的唯一运行内容来源。
- `my-skills/decisions/<skill-name>/`：该 Skill 的个人设计决策。
- `published-skills/<skill-name>/`：本机根据开发版生成的稳定运行快照，不是开发区；整个目录由 Git 忽略，不提交、不推送，也不作为跨机器源码来源。
...
2. 检查 `.source`、对应决策记录和本机当前发布版本，明确上游变化与个人变化。
...
7. 更新 `.source`，其中 `targets` 必须记录新机器首次发布时需要保留的安装平台；再运行适用的结构检查和行为验证。
...
发布是本机晋级动作，不是上传动作。Git 只同步 `my-skills/`、决策、测试、依赖锁和发布工具；另一台电脑拉取源码后，在当地重新执行发布和安装。

只有用户明确要求发布时，才可以更新 `published-skills/<skill-name>/`。发布时：

- 先展示开发版与发布版差异。
- 只复制运行所需文件，不复制 `.source` 或 `my-skills/decisions/`。
- 更新 `.release` 中的本机版本、来源、上游提交、内容哈希、发布时间和目标平台。跨机器判断运行内容是否一致时，以 `content_sha256` 为准，不以本机日期版本为准。
- 验证发布目录，而不只验证开发目录。
- 不自动安装、提交 Git、创建标签或推送；这些动作分别需要用户明确授权。发布目录本身即使已生成，也不得加入 Git。
